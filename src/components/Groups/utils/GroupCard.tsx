      ...
      <div className="relative overflow-hidden">
        ...
        <div className="absolute top-3 right-3 z-10 flex flex-col items-end gap-2">
          ...
          {group.privacy === GROUP_PRIVACY.CLOSED ? (
            <div className="flex items-center gap-1.5 rounded-full bg-red-600/90 px-2.5 py-1 text-[10px] font-bold tracking-wider text-white shadow-sm backdrop-blur-sm">
              <FaBan size={10} />
              CLOSED
            </div>
          ) : group.privacy === GROUP_PRIVACY.PRIVATE ? (
            <div className="flex items-center gap-1.5 rounded-full bg-gray-900/90 px-2.5 py-1 text-[10px] font-bold tracking-wider text-white shadow-sm backdrop-blur-sm">
              <FaLock size={10} />
              PRIVATE
            </div>
          ) : (
            <div className="flex items-center gap-1.5 rounded-full bg-green-600/90 px-2.5 py-1 text-[10px] font-bold tracking-wider text-white shadow-sm backdrop-blur-sm">
              <FaGlobe size={10} />
              PUBLIC
            </div>
          )}
          ...
          {group.type === GROUP_TYPES.OFFICIAL_INSTITUTION ? (
            <div className="flex items-center gap-1.5 rounded-full bg-purple-600/90 px-2.5 py-1 text-[10px] font-bold tracking-wider text-white shadow-sm backdrop-blur-sm">
              <FaUniversity size={10} />
              OFFICIAL
            </div>
          ) : (
            group.type === GROUP_TYPES.JOBS_CAREERS && (
              <div className="flex items-center gap-1.5 rounded-full bg-cyan-600/90 px-2.5 py-1 text-[10px] font-bold tracking-wider text-white shadow-sm backdrop-blur-sm">
                <FaBriefcase size={10} />
                CAREER
              </div>
            )
